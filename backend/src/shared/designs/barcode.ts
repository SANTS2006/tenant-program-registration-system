// Code 128 (character set B) symbol patterns: bar/space widths for values 0-106.
const PATTERNS = (
  ...
  "214121 412121 111143 111341 131141 114113 114311 411113 411311 113141 114131 311141 411131 211412 211214 211232"
).split(" ");
const START_B = 104;
const STOP = "2331112";

/** Encodes printable ASCII as Code 128B, returned as modules ("1" = bar), without quiet zones. */
export function code128(value: string): string {
  const chars = [...value].filter((ch) => ch.charCodeAt(0) >= 32 && ch.charCodeAt(0) <= 126);
  const codes = [START_B, ...chars.map((ch) => ch.charCodeAt(0) - 32)];
  const checksum = codes.reduce((sum, code, i) => sum + code * Math.max(i, 1), 0) % 103;
  const widths = [...codes, checksum].map((code) => PATTERNS[code]!).join("") + STOP;
  let bits = "";
  [...widths].forEach((w, i) => {
